...
<html>
   ...
   <body>
	<form name=consulta method=post action=regsub-sis2.php>
...
<?php
	...
	$today = date("Y-m-d");
		...
		$i=0;
 		...
 		$consulta="select conse,codeps,tipocabe,identicafabe,tipoideafi,ideafi,apellido1afi,apellido2afi,nombre1afi,nombre2afi,fenacimiento,sexo,tipoafi,parentesco,grupopobl,np,ficha,conben,coddpto,codmunicipio,area,afisistema,afiars,numcontrato,fecontrato,tipocontrato,grupoet,modsub,direccion,acuerdo,fechanove,estado,observacion from regsub where (np='1' or np='2') ORDER BY apellido1afi,apellido2afi,nombre1afi,nombre2afi";
	...
	$cont=0;
		...
		while($j<$i)
			...
			$consul="select apellido1,apellido2,nombre1,nombre2 from sisben where apellido1='$ape1[$j]' and apellido2='$ape2[$j]' and nombre1='$nom1[$j]' and nombre2='$nom2[$j]'";
...
<?php
	...
	echo"<table width=100%  border=0 class='encab'>";
		...
		echo"<tr>";
		echo"<td>Lista de registros de régimen subsidiado que no están en sisben por nombres (".$cont.")</td>";		    
		echo"</tr>";
	...
	echo"<tr>";
	echo"<td><a href=exregsub-sis2.php>Exportar datos a Excel</a></td>";
	echo"</tr>";
	echo"</table>";  
