...
<html>
   ...
   <body>
	<form name=consulta method=post action=homoregsubval.php>
...
<?php
	...
	$today = date("Y-m-d");
		...
		$i=0;
 		...
 		$consulta="SELECT conseafi,codeps,idecabefami,numidecabefami,tipoideafi,ideafi,apellido1,apellido2,nombre1,nombre2,fenacimiento,sexo,tipoafi,parentesco,grupopobla,nivelsisben,numficha,condicion,departamento,municipio,zona,feafiliacion,feeps,numcontra,iniciocontra,tipo,pertenencia,modalidad,estado FROM regsubvalido ORDER BY ideafi";
...
<?php
	...
	$cont=0;
		...
		while($j<$fil)
			...
			$consul="select ideafi from regsubvalido where ideafi='$ced[$j]'";
...
<?php	
	...
	echo"<table width=100%  border=0 class='encab'>";
		...
		echo"<tr>";
		echo"<td>Lista de homónimos de régimen subsidiado validado por documento (".$cont.")</td>";		    
		echo"</tr>";
	...
	echo"<tr>";
	echo"<td><a href=exphoregsubval.php>Exportar datos a Excel</a></td>";
	echo"</tr>";
	echo"</table>";
